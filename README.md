...
ZMK timings copied across: tapping term 200 ms; HRMs are balanced + chordal + hold-on-other-key-*release*, quick-tap 175 ms, prior-idle 150 ms (measured from the previous key’s press, as ZMK does). `C`/`V`/`H`/`[`/`]` are tap-preferred (hold only after 200 ms). Thumbs are the same balanced/permissive rule as HRMs (a roll off the thumb stays a tap; keep the thumb down and tap the other key for the layer). Scroll is 16 notches/sec, no accel.
